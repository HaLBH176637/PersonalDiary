...
public async Task<IActionResult> FilterPostsPublic(string sortOrder, string privacy, int page = 1, int pageSize = 5)
{
    List<Post> posts = new List<Post>();
    try
    {
        // Assuming you're using Entity Framework, get posts from your database directly
        var postsQuery = _context.Posts.AsQueryable(); // _context is your DbContext
        ...
        if (userId == null)
        {
            ...
            return View("Error"); // Or redirect to login page
        }

        // Filter posts based on privacy and userId
        if (privacy == "public")
        {
            postsQuery = postsQuery.Where(post => post.Privacy == "public" && post.UserId == userId);
        }
        else if (privacy == "private")
        {
            postsQuery = postsQuery.Where(post => post.Privacy == "private" && post.UserId == userId);
        }
        else
        {
            postsQuery = postsQuery.Where(post => post.UserId == userId); // For all user posts
        }

        // Sort posts based on the sortOrder parameter
        postsQuery = sortOrder switch
        {
            "newest" => postsQuery.OrderByDescending(post => post.CreatedAt),
            "oldest" => postsQuery.OrderBy(post => post.CreatedAt),
            "mostLikes" => postsQuery.OrderByDescending(post => post.Likes.Count),
            "leastLikes" => postsQuery.OrderBy(post => post.Likes.Count),
            _ => postsQuery.OrderByDescending(post => post.CreatedAt) // Default: newest first
        };

        // Get the filtered and sorted posts from the database
